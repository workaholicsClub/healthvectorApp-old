...
/**
 * Flat position -- flat list position, the position of an item relative to all the other <b>visible</b>
 * items on the screen. For example, if you have a three groups, each with 2 children and all are
 * collapsed, the "flat position" of the last group would be 2. And if the first of those three
 * groups was expanded, the flat position of the last group would now be 4.
 * <p>
 * This class acts as a translator between the flat list position, i.e. what groups and children
 * you see on the screen -- to and from the full backing list of groups & their children.
 */
public class ExpandableList {
     ...
     *
     * @param flatPos the flat list position to be translated
     ...
     */
    public ExpandableListPosition getUnflattenedPosition(int flatPos) {
        int groupItemCount;
        int adapted = flatPos;
        for (int i = 0; i < groups.size(); ++i) {
            groupItemCount = numberOfVisibleItemsInGroup(i);
            if (adapted == 0) {
                return ExpandableListPosition.obtain(ExpandableListPosition.GROUP, i, -1, flatPos);
            } else if (adapted < groupItemCount) {
                return ExpandableListPosition.obtain(ExpandableListPosition.CHILD, i, adapted - 1, flatPos);
            }
